...
//
// WHY THIS IS A SEPARATE FILE FROM project-selection.test.ts. Lanes F1 and D-20
// both wrote a test at that path (an add/add), and BOTH are kept -- but they
// cannot share a file. D-20's tests replace the whole client with
// `mock.module("@/lib/veridian-client", ...)`, which is process-global in bun;
// the assertions below exist precisely to prove which URL the REAL client
// requests, so they must run with the real module loaded. Merged into one file
// the mock would win and these four tests would silently stop testing
// anything -- passing, and meaningless. Split, both lanes keep their coverage.
//
