...
describe("a run lives in the URL (R67 E-09)", () => {
  test("reads every parameter a run is made of", () => {
    ...
    expect(run).toEqual({ report: "project-status", projectId: "p-1", from: "2026-01-01", to: "2026-09-02", weekStart: "2026-08-31" });
  });
  ...
  test("round-trips: writing a run back and reading it again gives the same run", () => {
    const run = { report: "weekly-project", projectId: "p-1", from: "2026-08-01", to: "2026-08-31", weekStart: "2026-08-03" };
  ...
  test("weekStart is only carried for the report that needs it -- an irrelevant parameter in a shared URL is noise", () => {
    const qs = reportRunSearchParams({ report: "project-status", projectId: "p-1", from: "2026-08-01", to: "2026-08-31", weekStart: "2026-08-03" });
