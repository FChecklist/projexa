...
// R67 C-06: the project row is a DOOR -- a real link that also fills the
// control strip. Correction C-14 recorded the old four flat KPI tiles as "do
// not navigate at all"; D-02 (below) replaced those four with the portfolio
// band, and the two of its cards that still match a DOORS entry are wired
// through it directly -- see the merge note on that band.
...
export default function DashboardHomeView({
  ...
  // R67 C-06: fills the control strip for the two KPI cards below that still
  // have a matching DOORS entry, and for the project row link.
  ...
  return (
    <>
      ...
      <div className="flex-1 space-y-6 p-6">
        ...
        {data && (
          <>
            {/* R67 MERGE (D-11): D-02 (audit R-004/R-009) replaced the four
                flat DashboardCards C-06 had wired as doors -- a bare number
                each, no baseline, no destination, and an "Active Projects"
                count whose registry label carried the literal string
                "(HARD-STOP TEST)" onto the live home -- with one primary KPI
                plus three supporting ones, each with a real baseline. That
                design is canonical and kept whole. C-06's OWN rule survives
                where a card still has a matching DOORS entry: Revenue and
                Spend map onto dashboard.total_revenue / dashboard.total_expenses
                exactly (same destination, `data-testid`-free onClick below
                does openDoor(..., {navigate:false}) then the same push), so
                a header button or composer chip that opens either door still
                agrees with this tile. Two doors are now unreachable from this
                screen (dashboard.active_projects, dashboard.total_budget --
                folded into the greeting and the primary card's baseline) and
                the new "Portfolio earned value" / "Permits expiring" cards
                have NO matching door yet (D-02 introduced them after DOORS
                was built) -- both navigate directly, honestly, rather than
                being forced through a door table entry invented for this
                merge. A door for the two is real follow-up work, not a
                mechanical rename. */}
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,2fr)]">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <KpiCard
                  ...
                  onClick={() => {
                    openDoor("dashboard.total_revenue", { navigate: false });
                    const door = doorById("dashboard.total_revenue");
                    router.push(door ? doorRoute(door, null) : "/invoices");
                  }}
                />
                <KpiCard
                  ...
                  onClick={() => {
                    openDoor("dashboard.total_expenses", { navigate: false });
                    const door = doorById("dashboard.total_expenses");
                    router.push(door ? doorRoute(door, null) : "/expenses");
                  }}
                />
                <KpiCard
                        ...
                        >
                          ...
                          <TableCell className="font-medium">
                            {/* R67 C-06: still a real link -- it now also
                                ...
                                "<project> > Dashboard > Project". ChainDoor
                                renders next/link itself, so the row's own
                                stopPropagation is unneeded here (the row
                                handler and the link both push the identical
                                URL). */}
