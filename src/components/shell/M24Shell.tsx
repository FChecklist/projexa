...
/**
 * The one adapter between R67 A-16's retry policy and F-01/F-13's cache.
 *
 * They report failure differently on purpose -- readJsonWithRetry returns it as
 * a value (so the retry rules stay pure and testable without exceptions), and
 * the shell store signals it by throwing (so a failure is never written to the
 * cache). This turns the first into the second, in one place, so all four shell
 * reads are BOTH retried and deduplicated rather than one or the other.
 *
 * ShellFetchError, not a bare Error: loadTasks distinguishes "the backend told
 * us why" from "we could not reach it at all", and that check is on the type.
 * Status 0 because a read that exhausted its attempts has no single response
 * status to report.
 */
async function readShellJson<T>(url: string): Promise<T> {
  const read = await readJsonWithRetry<T>(url);
  if (!read.ok) throw new ShellFetchError(read.error, 0);
  return read.data;
}
...
export default function M24Shell({ children }: { children: React.ReactNode }) {
  ...
  // src/lib/shell-cache.ts, which also collapses concurrent callers onto ONE
  // request.
  //
  ...
  //
  // R67 A-16 -- AND EACH ATTEMPT IS MADE TWICE BEFORE ANY FALLBACK. A single
  // failed read is usually a dropped request, not a broken backend: this
  // product runs on site connections and all four of the shell's reads happen
  // in the same first second of a page load, which is exactly when a flaky link
  // drops one. The retry policy is pure and lives in shell-resilience.ts; the
  // cache uses it as its READER (readShellJson below), so a retried read is
  // still deduplicated and still cached, and a read that fails BOTH attempts is
  // not cached at all.
  const loadOrgInfo = useCallback(async (options: { force?: boolean } = {}) => {
    try {
      ...
      const d = await cachedShellJson<OrgInfo>("shell:organization", "/api/organization", {
        force: options.force,
        ttlMs: SHELL_SESSION_TTL_MS,
        read: readShellJson,
      });
      setOrgFailed(false);
  ...
  const loadProjects = useCallback(async () => {
    try {
      ...
      // project invalidates SHELL_PROJECTS_KEY (see CreateProjectDialog), so a
      // new project still appears in the switcher immediately.
      const d = await cachedShellJson<Project[] | { projects?: Project[] }>(SHELL_PROJECTS_KEY, "/api/projects", {
        ttlMs: SHELL_SESSION_TTL_MS,
        read: readShellJson,
      });
      ...
      // first" sentence -- that would be a confident empty state standing in
      // for "not loaded yet", the exact defect this shell has been corrected
      // for twice already.
      setProjectsLoaded(true);
    } catch (err) {
      noteFailure("your projects", err instanceof Error ? err.message : "the request did not complete");
    }
  }, [noteFailure]);
  ...
  useEffect(() => {
    ...
    const { data: sub } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === "SIGNED_IN") {
        ...
        // A-16: the identity moves with the session too, so a sign-in as
        // somebody else must not leave the previous user's ranking on screen.
  ...
  const loadTasks = useCallback(async (options: { force?: boolean } = {}) => {
    ...
    try {
      ...
      // empty list.
      data = (await cachedShellJson<ApiTasks>("shell:tasks", "/api/tasks?limit=50", {
        force: options.force,
        read: readShellJson,
      })) ?? ({} as ApiTasks);
    } catch (err) {
      setTasksError(err instanceof ShellFetchError ? err.message : "Couldn't reach the task service.");
      return;
    }
    setTasksError(null);
    setCounts({
  ...
  // passed from.
  const loadRanking = useCallback(async (options: { force?: boolean } = {}) => {
    let d: PillPayload;
    try {
      d = await cachedShellJson<PillPayload>("shell:pill-usage", "/api/pill-usage?limit=6", {
        force: options.force,
        read: readShellJson,
      });
    } catch (err) {
  ...
  // frame with an empty list, and the pill strip renders A-16's per-user cached
  // ranking until the server's answer arrives. So they are scheduled AFTER the
  ...
  const onSubmit = useCallback(async () => {
    ...
    try {
      ...
      void loadRanking({ force: true });
    } catch {
      setSubmitError("Couldn't reach the task service.");
    } finally {
      setSubmitting(false);
    }
  // Both deps are real: main's body navigates via `router` on a verdict link,
  // and this lane's refreshes the ranking after a successful send. Dropping
  // either would be a stale-closure bug the linter is right about.
  }, [draft, pendingFunctionId, mode, projectId, chainModule, submitting, loadTasks, loadRanking, router]);
