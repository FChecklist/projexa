...
//
// The two meet cleanly: G-01 decides what a row SAYS, F-18 decides when the
// rows arrive. useModuleList replaced the useEffect/useState pair G-01 was
// written against, so those imports are gone and nothing else changed.
...
// G-01's rename of the last column ("Days left" -> "Status", because the cell
// now answers a question instead of promising a number) lives there too, for
// the same reason: a skeleton whose last head reads "Days left" handing over to
// a table whose last head reads "Status" is a header that rewrites itself.
