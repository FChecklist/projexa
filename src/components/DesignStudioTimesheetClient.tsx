...
// R67 D-61 (swept at the merge). formatNumber() applies fractionDigits as BOTH
// the minimum and the maximum, so these hours still read "8.00" and the
// tabular-nums column still lines up on the point -- with the locale pinned,
// which .toFixed(2) does not do for the grouping separator.
        ...
        <Card className="shadow-card">
          <CardContent className="p-0">
                ...
                {days.map((day) =>
                  day.rows.map((row, index) => (
                    <TableRow key={row.id}>
                      ...
                      <TableCell className="text-right tabular-nums">{formatNumber(row.hours, { fractionDigits: 2 })}</TableCell>
                      <TableCell>
            ...
            <div className="border-t border-px-border p-3 text-right text-sm font-medium text-px-ink">
              Total: {formatNumber(totalHours(rows), { fractionDigits: 2 })} hrs
            </div>
          </CardContent>
        </Card>
      )}
