...
// Real-screen conversion (2026-08-30): replaces PurchaseOrdersClient.tsx's
// old "New Purchase Order" Dialog popup with a real create screen. Posts to
// the same /api/purchase-orders this page already used (erp_purchase_orders
// via erp-buying-service.ts -- the SAME table/rows Procurement's own PO
...
export default function PurchaseOrderCreateClient() {
  ...
  async function create() {
    ...
    setSubmitting(true);
    try {
      const po = await fetchJson<{ id: string }>("/api/purchase-orders", {
      ...
      toast.error(errorMessage(err, "Couldn't create purchase order"));
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <ObjectScreen
      breadcrumb="Purchase Orders / New Purchase Order"
      ...
      saveDisabled={submitting || !vendorId}
      saveDisabledReason={submitting ? "Creating…" : !vendorId ? "Vendor is required" : undefined}
      messages={[]}
    >
