...
export default function MoMObjectClient({
  ...
  meetingId: string;
  /** ?focus=minutes -- arriving straight from Save on /moms/new. */
  ...
  // D-17: arriving from /moms/new lands the cursor in the minutes box and
  // names what was just made, with its number, in the persistent band.
  useEffect(() => {
    if (!meeting) return;
    if (focusMinutes) minutesBoxRef.current?.focus();
    if (justCreated) {
  ...
  }, [meeting?.id, meeting?.systemId, meeting?.title, focusMinutes, justCreated]);
      ...
      headerActions={
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem disabled={busy === "share"} onSelect={() => void createShareLink("whatsapp")}>
              Send on WhatsApp
            </DropdownMenuItem>
