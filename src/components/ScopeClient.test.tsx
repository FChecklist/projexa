...
describe("ScopeClient lineage grouping (D-23)", () => {
  ...
  test("shows BOTH signed variation columns, with the payload's own figures", async () => {
    ...
    expect(getByText("AED +2,025")).toBeDefined(); // Rev2 vs prior
    expect(getAllByText("AED -850").length).toBeGreaterThan(0); // Rev1, both columns
    expect(getByText("AED +1,175")).toBeDefined(); // Rev2 vs original
  ...
  test("a row with no figure gets an em dash titled 'Variation unavailable', never a fabricated AED 0", async () => {
    ...
    expect(unavailable[0].textContent).toBe("—");
  });

  test("status is a glyph plus a WORD, and superseded is not painted destructive", async () => {
    ...
    const superseded = getAllByText(/Superseded/)[0] as HTMLElement;
    expect(superseded.className).toContain("text-px-muted");
    expect(superseded.className).not.toContain("error");
  });
