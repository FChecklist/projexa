...
import { revisionLabel, variationCell } from "@/lib/boq-lineage";
...
export default function ScopeObjectClient({
  ...
  async function saveLineItemBudget(
    rowId: string,
    patch: { budgetPercentage?: number; vendorId?: string | null; vendorAmount?: number | null; materialAmount?: number | null; manpowerAmount?: number | null },
    cellKey: string
  ) {
    ...
    >
      ...
      {predecessor && (
        <div className="border-b border-ct-border px-4 py-2 text-[12.5px] text-px-ink">
          ...
          {(() => {
            const cell = variationCell(variationVsParent, currencyCode);
            return (
              <span className={cell.kind === "up" ? "text-px-success" : cell.kind === "down" ? "text-px-error" : "text-px-muted"} title={cell.title}>
                <span aria-hidden="true">{cell.glyph}</span>{cell.text ? ` ${cell.text}` : ""}
              </span>
            );
          })()}{" "}
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              {/* R67 D-26: the other two thirds of Sumeet's budget model. */}
