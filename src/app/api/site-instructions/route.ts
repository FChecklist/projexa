...
export const GET = withTiming("GET", async function GET(request: NextRequest) {
  ...
  try {
    ...
    return veridianErrorResponse(err, "Failed to load site instructions");
  }
});
...
export const POST = withTiming("POST", async function POST(request: NextRequest) {
  ...
  if (!contentType.includes("multipart/form-data")) {
    ...
    try {
      ...
      // R67 integration: the JSON branch classifies its failure through the
      // SAME helper the multipart branch below already uses (F-20), so the two
      // branches of one handler cannot report a timeout differently.
      return veridianErrorResponse(err, "Failed to create site instruction");
    }
  }
