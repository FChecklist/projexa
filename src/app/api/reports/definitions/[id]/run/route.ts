...
export async function POST(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    return NextResponse.json({ error: err instanceof VeridianApiError ? err.message : "Failed to run this report/analysis" }, { status: err instanceof VeridianApiError ? err.status : 502 });
  }
}
