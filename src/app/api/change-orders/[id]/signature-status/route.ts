...
export async function GET(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    return NextResponse.json({ error: err instanceof VeridianApiError ? err.message : "Failed to load signature status" }, { status: err instanceof VeridianApiError ? err.status : 502 });
  }
}
