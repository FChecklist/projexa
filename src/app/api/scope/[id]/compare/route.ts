...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ error: err instanceof VeridianApiError ? err.message : "Failed to compare scope revisions" }, { status: err instanceof VeridianApiError ? err.status : 502 });
  }
}
