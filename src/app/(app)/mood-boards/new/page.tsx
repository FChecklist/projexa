...
import MoodBoardCreateClient from "@/components/MoodBoardCreateClient";

export default async function MoodBoardNewPage({ searchParams }: { searchParams: Promise<{ projectId?: string }> }) {
    ...
    return (
      <div className="flex-1">
        <CreateScreenUnavailable
          breadcrumb="Mood Boards / New Mood Board"
          title="New Mood Board"
          backHref="/mood-boards"
          backLabel="Back to Mood Boards"
  ...
  return (
    <div className="flex-1">
      <MoodBoardCreateClient projectId={project.id} />
    </div>
  );
}
