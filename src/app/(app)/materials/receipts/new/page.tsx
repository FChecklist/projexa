...
const FRAME = {
  breadcrumb: "Materials / New Receipt",
  title: "New Receipt",
  backHref: "/materials",
...
async function ResolvedForm({ requestedProjectId }: { requestedProjectId?: string }) {
  ...
  return <MaterialReceiptCreateClient projectId={projectId} />;
}

export default async function MaterialsReceiptsNewPage({ searchParams }: { searchParams: Promise<{ projectId?: string }> }) {
  ...
  if (known) {
    return (
      <div className="flex-1">
        <MaterialReceiptCreateClient projectId={known} />
      </div>
    );
  }
