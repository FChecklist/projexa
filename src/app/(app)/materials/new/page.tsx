...
import MaterialCreateClient from "@/components/MaterialCreateClient";
...
async function ResolvedForm({ requestedProjectId }: { requestedProjectId?: string }) {
  ...
  return <MaterialCreateClient projectId={projectId} />;
}

export default async function MaterialsNewPage({ searchParams }: { searchParams: Promise<{ projectId?: string }> }) {
  ...
  if (known) {
    return (
      <div className="flex-1">
        <MaterialCreateClient projectId={known} />
      </div>
    );
  }
