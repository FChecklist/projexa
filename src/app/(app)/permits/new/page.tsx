...
async function ResolvedForm({ requestedProjectId }: { requestedProjectId?: string }) {
  ...
  return <PermitCreateClient projectId={projectId} />;
}

export default async function PermitsNewPage({ searchParams }: { searchParams: Promise<{ projectId?: string }> }) {
  ...
  if (known) {
    return (
      <div className="flex-1 p-6">
        <PermitCreateClient projectId={known} />
      </div>
    );
  }
