import FfeCreateClient from "@/components/FfeCreateClient";
import { resolveSelectedProject } from "@/lib/project-selection";
import CreateScreenUnavailable from "@/components/CreateScreenUnavailable";
import { getServerOrganizationId } from "@/lib/supabase/auth-guard";

// Real-screen conversion (2026-08-30): replaces the old "New Item" Dialog
// popup with a real create route.
export default async function FfeNewPage({ searchParams }: { searchParams: Promise<{ projectId?: string }> }) {
  ...
  return (
    <div className="flex-1">
      <FfeCreateClient projectId={project.id} />
    </div>
  );
}
