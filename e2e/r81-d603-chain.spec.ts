...
test("R-81 every visible pill is wired, and the module-chain population is hidden", async ({ page }) => {
  ...
  const leaked = mcTopLevel.filter((n) => n && pillNames.has(norm(n)));
  console.log(`R81_D603_R81_LEAKED ${JSON.stringify(leaked)} (exact-match; pills=${JSON.stringify([...pillNames])})`);
  expect(leaked, "a /api/module-chain chain is being offered as a visible pill").toEqual([]);
  ...
  for (let i = 0; i < total; i++) {
    ...
    if ((await btn.count()) === 0) {
      results.push({ label, effect: "GONE" });
      continue;
    }
    ...
    const effect = after !== before ? `nav:${after.replace("http://localhost:3100", "")}` : afterStrip !== beforeStrip ? "state-change" : "NONE";
  ...
  const dead = results.filter((r) => r.effect === "NONE" || r.effect === "GONE");
...
test("R-80 one full pill path works end to end", async ({ page }) => {
  ...
  // And it must carry a real data surface (a table, list rows, or cards), not
  // just a heading.
  const dataSurfaces = await page.locator("table tbody tr, [role=row], [data-slot=card]").count();
  console.log(`R81_D603_R80_DEST_ROWS ${dataSurfaces}`);
  expect(dataSurfaces, "the destination rendered no data surface at all").toBeGreaterThan(0);
});
