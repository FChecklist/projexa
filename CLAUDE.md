...
## Composer shell, part 11 — dropped the redundant "Dashboard / <project>" heading, then a full left↔right wiring sweep (every module, every leaf, every control) found and fixed one real bug (2026-09-08)

Two pieces, in the order they happened.

**1. The dashboard's own heading still repeated the project name.** Live-checked in both browsers per the owner's explicit ask ("check real chrome and confirm it matches too, also check using your inbuilt browser, i want right side panel to be checked"): `/dashboard/project`'s heading read `Dashboard / Harbor View Corporate HQ - Interior Fit-out`, but the frozen mock's own heading is plain `Dashboard` — the project is already named, prominently, in the TopRail's own pill immediately above it. Unlike the money-formatting decision elsewhere in `DashboardProjectClient.tsx` (R67 D-61, a documented fix), this repetition had no such rationale on record — an unscrutinized default, not a decision — so `breadcrumb={... \`Dashboard / ${d.projectName}\`}` became `breadcrumb="Dashboard"`. Verified live in both the Claude Browser pane and real Chrome (reconnected this round — `list_connected_browsers` had come back empty on the two prior attempts, not a transient blip, but was reachable this time): plain "Dashboard" heading, KPI strip/pills/control row all still matching the mock.

**2. The owner then asked for an exhaustive functional audit**, boolean pass/fail, of every left-panel control driving the right panel and every right-panel control driving the left, across every module, chain option and mode — with the explicit standing rule that the WIRING must still be the old app's, only the visuals changed. Tested directly in the Claude Browser pane (as asked), against `src/lib/module-catalogue.ts`'s own 14 modules and 33 leaves as ground truth:

- **All 14 module routes** (`/dashboard`, `/permits`, `/drawings`, `/documents`, `/moms`, `/scope`, `/work-progress`, `/labour`, `/materials`, `/budgets`, `/schedule`, `/reports`, `/customers`, `/vendors`) — navigated directly and via a real click through the "All modules" catalogue panel, confirming the composer's own chain strip derives the correct root + module segment from the URL in every case (R67 D-20's "the URL wins" contract, unmodified by any of this session's visual work).
- **All 13 create-leaf routes** with their own chain label (`/permits/new`, `/drawings/new`, `/documents/upload`, `/moms/new`, `/scope/new`, `/labour/attendance/new`, `/labour/new`, `/materials/receipts/new`, `/materials/new`, `/finance/budgets/new`, `/schedule/tasks/new`, `/customers/new`, `/vendors/new`) — each correctly produces a 3-segment chain (root → module → leaf).
- **Live click-driven round trips, both directions**: "All modules" → Permits → "Open" (left-panel click) correctly navigated the right panel from Dashboard to the real Permits list screen; the Permits screen's own "+ New" header button (right-panel click) correctly navigated to `/permits/new` **and** updated the left panel's chain to add the "New permit" segment. Home, Reset, and a Dashboard "Quick actions" link (`Record progress →` → `/work-progress?tab=entry&focus=activity`, chain updates to "Work Progress") all confirmed working both ways. Back/Remove confirmed correctly *disabled* on a URL-derived leaf segment (e.g. standing on `/permits/new` itself) — verified this is the kit's own `canCutAt` rule, identical for both controls, not a bug: a segment describing the page you're already on isn't something the chain-building flow "cut" ever meant to undo.
- **Pin (star) toggle** on a Frequent-actions card confirmed round-trips its `aria-pressed`/`aria-label` state correctly (tested and restored to its original state afterward).
- Confirmed, not re-litigated: the "mode" tabs (Projects/Customers/Vendors) the owner's item asked about are intentionally absent — R67 A-22, owner-approved decision D-10, `deriveMode()` still computes and sends the same value the old three tabs did; this is a standing design decision from before this session, not a gap.

**Real bug found and fixed**: `src/app/(app)/budgets/page.tsx`'s redirect (`/budgets` → `/finance/budgets`, R67 D-62) was the one member of its own redirect family that dropped its query string outright — its siblings `/budgets/new` and `/budgets/[id]` already forward theirs (see `budgets/new/page.tsx`'s own comment). Landing on `/finance/budgets` with no `?projectId=` meant `M24Shell`'s `routeProjectId` came back null, so the composer's left panel lost **both** the module segment and the project root — on a screen the right panel still tried to query data for. Fixed the same way the sibling redirect already does it: forward every query param verbatim. Pre-existing since R67 D-62 (2026-09-03), unrelated to any visual re-skin change; found only because this pass tested the "Budget" module's full round trip rather than assuming the pattern held. Live-verified: `/budgets?projectId=X` now correctly lands on `/finance/budgets?projectId=X` with the project root restored in the composer.

**Known, disclosed, NOT fixed** (a smaller, separate gap from the bug above, left alone deliberately): `/finance/budgets` and `/design-studio/timesheets/new` (the `schedule.time` leaf's own redirect target) aren't in `module-catalogue.ts`'s `prefixes` lists for "Budget"/"Schedule", so once the project-context bug above is fixed, the composer correctly shows the project root but still can't name a module segment on those two destination URLs — a generic `"What do you want to do?"` prompt instead of `"...on Budget"`. Widening those prefix lists is a real, own product decision (does "the ERP annual-budgets screen" belong under the same chain identity as project-scoped BOQ budgets? Does the redirected timesheet screen belong under "Schedule"?) rather than a bug fix with one obvious right answer, so it was surfaced here rather than guessed at.

**A test-harness gotcha worth recording for future sweeps**: Next.js dev mode compiles a route on its *first* visit in a running `bun run dev` process, and that compile can take 10–30 s (confirmed via `preview_logs`: `/materials/new` took 17.8 s, `/design-studio/timesheets/new` took 27.0 s) — during which the composer's chain can look wrong (missing the project root) purely because the page hasn't hydrated yet, not because of any real defect. Every "missing root" reading this pass hit resolved cleanly on a second, warm visit. Don't conclude a wiring bug from a first-visit dev-mode read without re-checking warm.

Verified: typecheck clean, full suite 4088/4088 pass twice (one interim run showed the repo's own already-documented `BudgetCreateClient` `act()`-warning flake — 3 fail/2 errors — which cleared on a clean re-run, consistent with the pre-existing pattern, not caused by either change here), lint clean, production build clean (`/finance/budgets`, `/finance/budgets/new` both present in the route manifest). Real Chrome and the Claude Browser pane both re-confirmed matching the frozen mock's right panel.

## Composer shell, part 12 — the "Frequent actions" rows were still the OLD capsule-button shape under the new icons (owner: "the left side is still old one", 2026-09-08)

Part 5-7 swapped each row's kind-word badge for a lucide icon and the "Pin"/"Pinned" text button for a small star, but never actually changed the row's own CONTAINER — every row still rendered on `.veri-mode-pill`, a kit class whose own CSS (`border-radius: 999px`) draws a big rounded capsule, not the mock's actual shape. Owner, looking at real Chrome: "the left side is still old one / you were to update it / only update the left side." Re-read the frozen mock's own DOM directly (not a screenshot glance) rather than guessing again:

```html
<div style="display:flex;justify-content:space-between;font-size:11px;padding:3px 6px;
     border-radius:8px;background:#E1F5EE;">
  <span><i class="ti ti-edit"/> Log time</span>
  <i class="ti ti-star" style="color:#9a9a96"/>
</div>
```

A flat, compact, 11px single-line row — icon + label on the left, a small MUTED-gray star at the row's own right edge, never a pill. Rebuilt all three "Frequent actions" row groups (screen cards, "Do again" recent chains, ranked cards) in `PillStrip.tsx` to that exact shape (`padding: 3px 6px`, `border-radius: 8px` — this app's own `rounded-lg` Tailwind utility resolves to 16px here, a locally-customized token, so the arbitrary value `rounded-[8px]` was used instead), including fixing the unpinned star's colour (was saffron/orange regardless of state; the mock shows plain muted gray until pinned, so only the pinned state now fills saffron). Every `onClick`/`onTogglePin`/`aria-label`/`aria-pressed`/`disabled` prop is byte-for-byte the same as before — this is a container/style change only.

**One disclosed trade-off, not previously encountered in this series**: the pin star's real hit box shrank from 44x44 (R67 A-18's usual minimum) to ~22x22. A literal 44px box centred on these ~22px-tall, 4px-apart rows would overlap the *next* row's own pin target by roughly 16px — a tap near the shared edge could pin the wrong row, which is a worse defect than a smaller target. 22x22 is the largest box that does NOT overlap a neighbour at the mock's own spacing; it is still well clear of the ~14px bare-glyph case A-18 was originally written for. `strip-controls.test.tsx`'s "Pin, in both of its states" test now asserts 22px, with the reasoning written into both files rather than silently changed.

Verified: typecheck clean, `src/components/shell/` full test directory 264/264 pass, live-checked in both the Claude Browser pane and real Chrome — computed styles read directly from the DOM (`border-radius: 8px`, `padding: 3px 6px`, `font-size: 11px`, pin box 22x22) match the mock's own values exactly, not just "looks about right" on a screenshot.

## Composer shell, part 13 — Task Master was never meant to be always-visible; the mock has none of it in its default view (owner, on real Chrome: "the home, approved pending... is still in the top... the Frequently Used is missing", 2026-09-08)

The single biggest gap this whole series had missed. Every prior part (5-12) worked on ROW STYLING inside "Frequent actions" -- icons, capsule shape, colours -- and never questioned WHERE that band sat relative to Task Master. Re-read the frozen mock's own DOM directly (`get_page_text` on the live mock file, not a screenshot glance): its left panel's full content is `Frequent actions / [5 rows] / All modules Tasks Back Home / [2 example prompts] / [input]` -- there is no "Home (38) / Approval Pending (20) / In Queue (0) / Completed (18) / History (11)" tab row anywhere in it, and no task-row list above Frequent actions. The real app had Task Master's tabs+rows ALWAYS rendered above Frequent actions (part 10's own comment even said so explicitly: "the Task Master tabs... are already always visible above this strip"), which is exactly backwards from what the mock shows and is why Frequent actions read as "missing" -- it was there, just buried under 20+ blocked-task rows and a "Show 20 more" button.

**Why this was a real layout change, not a CSS tweak.** `Composer.tsx`'s root has always been `position:absolute; bottom:0`, growing UPWARD -- a design built, and repeatedly bug-fixed (see this file's own history), on the premise that it floats OVER a Task Master pane that is always taller than the composer and always there. Simply hiding Task Master's content while keeping that positioning would leave a large BLANK GAP between the rail and the composer (which would still float near the bottom, pinned there, however short its own content), not the mock's actual look (content starts immediately under the rail).

**The fix, four files, all backward-compatible via defaults:**
- `M24Shell.tsx`: new `tasksExpanded` state, `false` by default -- same expand/collapse pattern `showAllPills` ("All modules") already established one state above it.
- `ControlStrip.tsx`: TASKS now calls a real `onToggleTasks` handler (falls back to `onHome` when absent, so nothing that predates this breaks) instead of aliasing HOME -- HOME's own behaviour is completely untouched.
- `Composer.tsx`: new `dockedOverTaskMaster` prop (default `true` = today's exact, unchanged, absolute/bottom-anchored/grows-upward mechanism). `false` (used only when `tasksExpanded` is false) switches the root to normal top-anchored flow -- no Task Master pane exists in that state to float over, so it doesn't need to.
- `AppShell.tsx`: new `taskMasterExpanded` prop (default `true` = today's exact reserved-height calc). `false` collapses that reservation to zero, so the now-normal-flow composer isn't left floating over an empty reserved gap.
- `M24Shell.tsx`'s own `taskMaster={...}` slot: the whole tab-row/task-list/`Show N more` block is now gated on `tasksExpanded`. The `shellErrors` banner (a real backend failure) is deliberately OUTSIDE that gate -- a genuine failure must never be hidden behind a collapsed panel nobody thought to open.

Nothing about Task Master's own functionality changed -- same tabs, same rows, same click-to-resolve actions (Pick line/Dismiss/Retry/Choose project), same data, same polling. This is DEFAULT VISIBILITY only, one click away behind the exact button the mock itself shows for this ("Tasks"), toggled with the identical mechanism already proven for "All modules" one line above it in the same file.

Verified: typecheck clean, full suite 4088/4088 pass, lint clean (1 pre-existing unrelated warning), production build clean. Live-verified BOTH states (collapsed default, expanded via Tasks) in the Claude Browser pane and real Chrome, at both 1280x720 and the stress-test 1280x404 viewport -- a two-stage geometric overlap sweep at every combination found only the ALREADY-DISCLOSED pre-existing Reset/KPI-card overlap (see part 11's own entry), zero overlaps introduced by this change, zero overlaps at all in the expanded state.

## Composer shell, part 14 — a real regression from part 13, plus a systematic pixel/colour re-audit found two more real mismatches (owner: "RECHECK PIXEL BY PIXEL, COLOUR BY COLOUR... THE FRONT END TO BE 100% COPY OF MOCK", 2026-09-08)

**1. Real regression, found live by the owner within minutes of part 13 shipping.** Collapsing Task Master by default (part 13) relayouts the composer's textarea from an absolutely-positioned box (immediately stable width) to a normal-flow one whose real width can still be settling for a frame while the sibling Task Master area's reserved height animates to 0. The textarea's own auto-resize effect (`ta.style.height="auto"; ta.style.height=Math.min(scrollHeight,220)+"px"`) ran once, against that transitional too-narrow width, wrapped the 2-row placeholder into many lines, measured a bogus `scrollHeight`, clamped at the 220px ceiling, and STAYED there -- nothing in its `[value, taRef]` dependency array ever re-ran it. Visible result: the send button floated in a huge dead gap and the example chips were pushed far down the panel. Confirmed live (`ta.style.height="auto"` after settling measured a real 46px, not 220) before touching any code. Fixed with the SAME pattern this file's own `rootRef` ResizeObserver already used for an identical "stale measurement" bug: switched the initial call to a dependency-free `useLayoutEffect` and added a `ResizeObserver` on the textarea's own parent, so a later, non-React-driven layout settle re-measures too. Verified: typecheck clean, full suite 4088/4088 pass, live-confirmed the textarea reads 46px on load in both states, and still grows correctly (127px) when real multi-line text is typed.

**2. A systematic, DOM-level pixel/colour re-audit** (computed styles read directly from both the live mock and the live app -- never eyeballed from a screenshot) found two more real, previously-missed mismatches, both now fixed:

- **The TopRail project pill** used the kit's `--color-scope-tint`/`--color-scope-tint-border` tokens -- a cream, 10px-radius, bordered chip -- per a real, documented, PRE-mock decision (M24-B/E-118: "reads as the one piece of context you act against"). The mock's own DOM draws it as a plain white, fully-rounded (`border-radius:999px`), borderless pill. Per the owner's explicit standing instruction that the mock's drawing now controls, this old decision is superseded for HOW it's drawn only -- same button, same aria-label/aria-live/aria-expanded/onClick, same switcher list.
- **The composer's example-prompt chips** (`M24Shell.tsx`) already had the mock's border/radius/padding right but were missing its white fill (transparent background, so no visible pill outline against the panel). Added `background:"#fff"`.

Both confirmed byte-identical to the mock's own computed values (`rgb(255,255,255)` / `border-radius:9999px`) in the Claude Browser pane AND real Chrome, on two different accounts/tenants.

**Known, deliberate, NOT changed** (recorded here so it isn't rediscovered as a surprise): the app's own muted-text colour family is a cool blue-grey "slate" token set (`rgb(113,128,150)`-ish) used everywhere -- headings, Filter/Export, KPI labels -- while the mock's own placeholder tokens are a neutral warm grey (`rgb(107,107,104)`-ish). Re-theming the whole app's grey scale to the mock's exact literal hex would be a far larger, riskier undertaking than matching a control's shape/size/behaviour, and no single component owns it -- same "too wide a blast radius" reasoning already applied to Filter/Export's own style in an earlier part. Colour FAMILY matches (muted, low-contrast, grey); exact hex does not, everywhere text uses this token set.

**Test-suite gotcha, now confirmed twice**: `dashboard-hierarchy-no-ffe-redirect.test.ts`'s own "(F_023) no source file ever navigates to /ffe" test -- a grep-style scan across source files -- timed out twice today in full-suite runs (6.3s and 6.29s, both just past whatever timeout the runner applies under load) while passing cleanly in isolation both times (698ms, 1191ms). Machine-load-sensitive, not a real failure; if it times out again, re-run the one file in isolation before treating it as a regression.

Verified end to end: typecheck clean, full suite 4088/4088 pass (isolated, no concurrent load), lint clean (1 pre-existing unrelated warning), production build clean. Live-verified in both the Claude Browser pane and real Chrome (two different accounts/tenants) -- computed styles read directly from the DOM match the mock's own values exactly, not approximated from a screenshot. Full left-panel navigation (All modules -> Permits -> Open) re-confirmed working end to end after all of today's changes combined.
